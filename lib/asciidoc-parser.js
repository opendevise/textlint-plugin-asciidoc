...
module.exports = class AsciiDocParser {
  ...
  parse () {
    ...
    let block
    while ((block = this.readBlock(this.readInterveningLines()))) {
  ...
  readBlock (dataLines, opts = { sections: true }) {
  ...
  readSection (level, dataLines) {
    ...
    while ((block = this.readBlock(this.readInterveningLines(section), { sectionLevel: level }))) {
  ...
  readDelimitedBlock (dataLines, opts = {}) {
    ...
    if (supportsBlockChildren) {
      let childBlock, dataLines = this.readInterveningLines(block),
          childOpts = { sections: false, enclosures: [startDelimiterLine].concat(opts.enclosures || []) }
